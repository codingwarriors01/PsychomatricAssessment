      ...
      <html>
        ...
        <div class="card" style={{ width: "35rem", border: "none" }}>
          <div class="card-body">
            ...
            <Link to='/exam_dashboard' className="btn btn-primary" style={{ visibility: id === 5 ? 'visible' : "hidden", marginLeft: "40px" }} onClick={() =>{saveAndNext();notify() }}>Submit</Link>
            <button className="btn btn-dark" style={{ visibility: id === 5 ? 'hidden':'visible', marginLeft: "40px" }} onClick={() => {dispatch({type:'NEXT',payload: props.updateProps(Number(props.id) + 1 < 11 ? Number(props.id) + 1 : 10)});saveAndNext();notify()}}>Save & Next &rarr; </button>
          </div>
        </div>
